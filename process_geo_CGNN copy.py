...
def K_fold(dataset, seed, cv=5, collate_fn=Atomgraph_collate, dataloader_param=dataloader_param, num_workers=0, pin_memory=False):
    ...
    split_data = []
    for i in range(cv):
        split = {"train": indices[i * step:i * step + step], "val": indices[0:i * step] + indices[i * step + step:total_size]}
        
        split_data.append(split)
        
        test_sampler = SubsetRandomSampler(split['train'])
        
        train_sampler = SubsetRandomSampler(split['test'])
...
def main(device, model_param, optimizer_param, scheduler_param, dataset_param, dataloader_param,
    ...
    train_data,test_data,split_data=K_fold(dataset=dataset)
