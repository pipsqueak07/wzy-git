...
def main(data_dir, data_name,output_path,name_database ,cutoff,max_num_nbr,compress_ratio,chunk_size=10000):
    ...
    for n, chunk in enumerate(tqdm(py_.chunk(data_files[:int(compress_ratio*len(data_files))], chunk_size))):
        ...
        '''
        first change the crystal as a graph;
        graph_lattice  means the crystal lattice;
        graph_names means the material id;
        graph_nodes means the atom feature, which is a matrix for every atom in this crystal:[N,M],N denotes the number of atom, M denotes the elements informations
        graph_edges means the bond feature, which is a matrix for bond in this crystal:
        nbr_fea_idx means the index of 12 neighbor atom: [M,12]
        nbr_distance means the direct distance between two atoms: [M,12]
        nbr_subtract means the distance of axis between two atoms: [M,12,3] (Euclidean norm)
        '''
        ...
        np.savez_compressed(os.path.join(output_path,"my_graph_data_{}_{}_{}_{}_{}_{:03d}.npz".format(data_name,name_database,int(cutoff),max_num_nbr,int(compress_ratio*100),n)), graph_dict=graphs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_name', metavar='N', type=str, default='data',
