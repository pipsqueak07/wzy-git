...
def create_model(device, model_param, optimizer_param, scheduler_param, load_model, model_name):
    ...
    return Model(device, model, name, model_name, optimizer, scheduler, clip_value)


def main(device, model_param, optimizer_param, scheduler_param, dataset_param, dataloader_param,
         num_epochs, seed, load_model, pred, pre_trained_model_path, model_name):
    ...
    if test_ratio == -1:
        ...
        split = {"train": indices[0:n_train], "val": indices[n_train:n_train + n_val],
                 "test": indices[n_train + n_val: n_graph]}
    ...
    if not pred:
        ...
        train_dl = DataLoader(dataset, sampler=train_sampler, pin_memory=False, **dataloader_param, drop_last=True)
        trainD = [n for n in train_dl]
        val_dl = DataLoader(dataset, sampler=val_sampler, pin_memory=False, **dataloader_param)
        print("the number of training set:   ", np.shape(split["train"]))
        print('start training')
#        model.train(train_dl, val_dl, num_epochs)
        model.train(train_dl, train_dl, num_epochs)
    ...
    test_dl = DataLoader(test_set,pin_memory=False, **dataloader_param)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", type=str, default='density.pth')
    ...
    main(device, model_param, optimizer_param, scheduler_param, dataset_param, dataloader_param,
         ...
         options["pre_trained_model_path"], options['model_name'])
